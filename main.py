import os
os.environ['TF_CPP_MIN_LOG_LEVEL']='2'#! python3
...
""")
    ...
    if args.example and args.dataset and args.split:
        try:
            mod_name = "{}.{}_{}_{}".format(args.example, args.split, args.dataset, args.loss)
...
if __name__ == "__main__":
    ...
    parser.add_argument('dataset', nargs="?", choices=['mnist', 'kdd','cc'], help='the name of the dataset you want to run the experiments on')
    parser.add_argument('split', nargs="?", choices=['run'], help='train the example or evaluate it')
    parser.add_argument('loss', nargs="?", default ='normalloss' ,choices=['newloss','normalloss'], help='with new loss or not')
