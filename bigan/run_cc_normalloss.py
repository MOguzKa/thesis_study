...
import importlib
import sys
import bigan.cc_utilities as network
import data.cc as data
...
def create_logdir(method, weight, rd):
    ...
    return "bigan/train_logs/cc_normalloss/{}/{}/{}".format(weight, method, rd)
...
def train_and_test(nb_epochs, weight, method, degree, random_seed):
    ...
    with tf.name_scope('loss_functions'):
        ...
        # loss_generator1 = tf.reduce_mean(-1*(x_gen.log_prob(input_pl))) #layerlari probability yapip bunu yazsam olacak mi acaba!!!!!??????
    ...
    with tf.name_scope('optimizers'):
        ...
        # optimizer_gen1 = tf.train.AdamOptimizer(learning_rate=learning_rate, beta1=0.5, name='gen_optimizer1')
    ...
    with sv.managed_session() as sess:
        ...
        while not sv.should_stop() and epoch < nb_epochs:
            ...
            train_loss_dis, train_loss_gen, train_loss_enc, train_loss_gen1 = [0, 0, 0, 0]
            print('normalloss running')
            # training
            for t in range(nr_batches_train):
                ...
                # _, lg1,sm1 = sess.run([train_gen_op1,
                #                       loss_generator1,
                #                       sum_op_gen1],
                #                       feed_dict=feed_dict)
                lg1 = 0
            ...
            print("Epoch %d | time = %ds | loss gen = %.4f | loss gen1 = %.4f| loss enc = %.4f | loss dis = %.4f "
                  % (epoch, time.time() - begin, train_loss_gen, train_loss_gen1, train_loss_enc, train_loss_dis))
        ...
        fill = np.ones([batch_size - size, 29])
        ...
        per = np.percentile(scores,20)

        res = pd.concat([pd.Series(scores),pd.Series(testy)],axis=1)

        #res.to_csv('/Users/oguzkaplan/Documents/repo/thesis/notebooks/bigan_cc_scores_normalloss_corr_32.csv')
        ...
        print(
            "Testing : Prec = %.4f | Rec = %.4f | F1 = %.4f | Acc = %.4f"
