...
def create_logdir(method, weight, rd):
    ...
    return "bigan/train_logs/kdd_normalloss/{}/{}/{}".format(weight, method, rd)
...
def train_and_test(nb_epochs, weight, method, degree, random_seed):
        ...
        with tf.variable_scope('Discriminator_loss'):
            if method == "cross-e":
                dis_score = tf.nn.sigmoid_cross_entropy_with_logits(
                    labels=tf.ones_like(l_generator_ema),logits=l_generator_ema) # logit modelden cikan degerdir yani prediction
    ...
    with sv.managed_session() as sess:
        ...
        while not sv.should_stop() and epoch < nb_epochs:
            ...
            print("Epoch %d | time = %.4fs | loss gen = %.4f | loss enc = %.4f | loss dis = %.4f "
        ...
        per = np.percentile(scores,50) # ne kadar percette varsa datada o kdardan ayirsun

        res = pd.concat([pd.Series(scores),pd.Series(testy)],axis=1)

        #res.to_csv('/Users/oguzkaplan/Documents/repo/thesis/notebooks/bigan_kdd_scores_normalloss_corr_32_egbad_like.csv')
