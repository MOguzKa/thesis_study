...
import importlib
import sys
import gan.kdd_utilities_newloss as network
...
def train_and_test(nb_epochs, weight, method, degree, random_seed):
    ...
    with tf.name_scope('loss_functions'):
        ...
        #loss_generator1 = tf.reduce_mean(tf.keras.losses.MSLE(input_pl,generator))
        loss_generator1 = tf.reduce_mean(tf.keras.losses.MSE(input_pl,generator))
        #loss_generator1 = tf.reduce_mean(-(generator.log_prob(input_pl))) #layerlari probability yapip bunu yazsam olacak mi acaba!!!!!??????
    ...
    with sv.managed_session() as sess:
        ...
        while not sv.should_stop() and epoch < nb_epochs:
            ...
            train_loss_dis, train_loss_gen, train_loss_gen1 = [0, 0, 0]
            ...
            print("Epoch %d | time = %ds | loss gen = %.4f | loss gen1 = %.4f| loss dis = %.4f "
                  % (epoch, time.time() - begin, train_loss_gen, train_loss_gen1,train_loss_dis))
        ...
        res.to_csv('/Users/oguzkaplan/Documents/repo/thesis/notebooks/gan_kdd_scores_newloss_method3.csv')
